        ...
        switch (action) {
          ...
          case 'reorder': {
            ...
            if (action === 'create') {
              ...
              const idx = updated.findIndex((s: any) => s.status === status_name);
              if (idx === -1) throw new Error(`Status "${status_name}" not found`);
              ...
              updated = updated.filter((s: any) => s.status !== status_name);
