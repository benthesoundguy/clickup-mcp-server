...
export function setupViewTools(server: McpServer): void {
  server.tool(
    ...
    async ({ action, list_id, view_id, name, type, filters, group_by, group_direction, group_collapsed,
             sort_by, sort_direction, settings, include_content, sharing_action, sharing_type, sharing_id,
      ...
      try {
        switch (action) {
          ...
          case 'duplicate': {
            if (!view_id || !name) throw new Error('view_id and name required for duplicate');
