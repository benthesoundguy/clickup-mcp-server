...
export function createProjectIntelligenceClient() {
  return {
    ...
    async health(listId: string) {
      ...
      const statuses = (list as any).statuses || [];
      ...
      for (const t of tasks) {
        ...
        if (!isClosed) {
          ...
          if (t.date_updated && (now - parseInt(t.date_updated)) > sevenDays) staleCount++;
          // Check dependencies from task object
          const deps = (t as any).dependencies || [];
          if (deps.length > 0) blockedCount++;
        }
    ...
    async dependencyAnalysis(listId: string) {
      ...
      let totalDeps = 0, maxChainDepth = 0, circularDeps: string[] = [];
    ...
    async sprintReadiness(listId: string) {
      ...
      for (const t of tasks) {
        ...
        if (['done', 'closed', 'complete'].includes(s)) holding.push(t);
        else if (hasBlockingDeps) blocked.push({ ...t, blocked_by: deps.map((d: any) => ({ id: d.depends_on, name: taskMap.get(d.depends_on)?.name, status: taskMap.get(d.depends_on)?.status?.status })) });
        else if (['in progress', 'in review', 'active'].includes(s)) inProgress.push(t);
        else ready.push(t);
      }
    ...
    async workload(listId: string) {
      ...
      for (const t of tasks) {
        ...
        if (assignees.length === 0 && !isClosed) unassigned++;
        ...
        for (const a of assignees) {
          ...
          if (isClosed) workloadMap[name].done++;
          else if (isBlocked) workloadMap[name].blocked++;
          else if (['in progress', 'in review', 'active'].includes(s)) workloadMap[name].in_progress++;
    ...
    async risk(listId: string) {
      ...
      for (const t of tasks) {
        const s = t.status?.status?.toLowerCase() || '';
        if (['done', 'closed', 'complete'].includes(s)) continue;
        ...
        // Stale (20%)
        if (t.date_updated && (now - parseInt(t.date_updated)) > 7 * 24 * 60 * 60 * 1000) { score += 20; drivers.push('stale'); driverCounts.stale++; }
        ...
        const chainDeps = deps.filter((d: any) => {
          const dt = taskMap.get(d.depends_on);
          const subDeps = (dt as any)?.dependencies || [];
    ...
    async timeReport(listId: string, start_date?: string, end_date?: string) {
      ...
      const taskMap = new Map((taskRes.tasks || []).map((t: any) => [t.id, t.name]));
      ...
      return {
        ...
        hours_per_person: Object.entries(perPerson)
          .map(([name, ms]) => ({ name, hours: Math.round((ms as number) / 3600000 * 100) / 100 }))
          .sort((a, b) => b.hours - a.hours),
        hours_per_task: Object.entries(perTask)
          .map(([name, ms]) => ({ name, hours: Math.round((ms as number) / 3600000 * 100) / 100 }))
          .sort((a, b) => b.hours - a.hours),
        hours_per_day: Object.entries(perDay)
          .map(([day, ms]) => ({ day, hours: Math.round((ms as number) / 3600000 * 100) / 100 }))
