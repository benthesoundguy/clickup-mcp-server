...
export function setupDependencyTools(server) {
    // ── Tool 1: dependencies — task-level CRUD + analysis ─────────────────
    server.tool('dependencies', 'Manage task dependencies. Use action to create, get, update, delete, view dependency graph, check/resolve conflicts, view timeline impact, or bulk create.', {
    ...
    server.tool('dependencies_workspace', 'Manage dependencies at the workspace level. Use action to list all dependencies, view stats, or export/import the dependency graph.', {
        ...
        try {
            switch (action) {
                ...
                case 'import': {
                    if (!data)
                        throw new Error('data required for import');
