...
export function setupTemplateTools(server) {
    server.tool('templates', 'Get available templates in a ClickUp workspace. Scope type determines which templates are returned (task, list, or folder templates).', {
        ...
        try {
            switch (scope_type) {
                ...
                case 'list': {
                    if (!scope_id)
                        throw new Error('scope_id is required for list templates');
                    const result = await templatesClient.getListTemplates(workspace_id, scope_id);
                    return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
                }
                case 'folder': {
                    if (!scope_id)
                        throw new Error('scope_id is required for folder templates');
