...
export function setupReminderTools(server) {
    server.tool('reminders', 'Manage personal reminders in ClickUp. Use action "list" to search/filter, "create" to add, or "update" to modify or complete.', {
            ...
            switch (params.action) {
                ...
                case 'create': {
                    if (!params.title || !params.due_date)
                        throw new Error('title and due_date are required for create');
                    const result = await remindersClient.createReminder(params.title, params.due_date, params.description);
                    return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
                }
                case 'update': {
                    if (!params.reminder_id)
                        throw new Error('reminder_id is required for update');
                    const result = await remindersClient.updateReminder(params.reminder_id, params.title, params.description, params.due_date, params.is_completed);
                    return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
                }
            }
        }
...
//# sourceMappingURL=reminder-tools.js.map