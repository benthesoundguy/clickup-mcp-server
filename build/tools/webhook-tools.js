...
export function setupWebhookTools(server) {
    server.tool('webhooks', 'Manage ClickUp webhooks and parse incoming webhook events. Use action to list, create, update, delete webhooks, or process an incoming payload with optional HMAC validation.', {
        ...
        try {
            switch (action) {
                ...
                case 'create': {
                    if (!workspace_id || !endpoint || !events?.length)
                        throw new Error('workspace_id, endpoint, and events required for create');
                    const result = await webhooksClient.createWebhook(workspace_id, endpoint, events, space_id);
                    return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
                }
                case 'update': {
                    if (!webhook_id)
                        throw new Error('webhook_id required for update');
                    const result = await webhooksClient.updateWebhook(webhook_id, endpoint, events, status);
                    return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
                }
                case 'delete': {
                    if (!webhook_id)
                        throw new Error('webhook_id required for delete');
                    await webhooksClient.deleteWebhook(webhook_id);
                    return { content: [{ type: 'text', text: JSON.stringify({ success: true }, null, 2) }] };
                }
                case 'process': {
                    if (!payload)
                        throw new Error('payload is required for process');
