...
export function setupTimeTrackingTools(server) {
    server.tool('time_entries_list', 'Get time entries for a ClickUp team/workspace. Supports filtering by task, date range, and assignees.', {
    ...
    server.tool('time_entry_create', 'Create a new manual time entry for a task. Requires team_id, task_id, and start time. Optional duration or end_time.', {
    ...
    server.tool('time_entry_update', 'Update an existing time entry. Supports changing start, duration, end time, description, and billable status.', {
    ...
    server.tool('time_entry_delete', 'Delete a time entry by ID.', {
        team_id: z.string().describe('The ID of the team (workspace) containing the time entry'),
    ...
    server.tool('time_tracking_start', 'Start the timer on a ClickUp task. Only one timer can run at a time. Supports optional description and billable status.', {
    ...
    server.tool('time_tracking_stop', 'Stop the currently running timer. Returns the completed time entry details.', {
    ...
    server.tool('time_tracking_current', 'Get the currently running time entry, if any. Returns the active time entry details or an empty response.', {
...
//# sourceMappingURL=time-tracking-tools.js.map