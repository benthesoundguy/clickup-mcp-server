...
export function setupCustomFieldValueTools(server) {
    server.tool('custom_fields_values', 'Get, set, remove, or bulk-set custom field values on ClickUp tasks. '
        ...
        value: z.any().optional().describe('Required for set: the value. Format depends on field type:\n'
        ...
        try {
            switch (action) {
                ...
                case 'set': {
                    if (!task_id || !field_id)
                        throw new Error('task_id and field_id required for set');
                    const result = await customFieldsClient.setTaskFieldValue(task_id, field_id, value);
                    return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
                }
                case 'remove': {
                    if (!task_id || !field_id)
                        throw new Error('task_id and field_id required for remove');
                    await customFieldsClient.removeTaskFieldValue(task_id, field_id);
                    return { content: [{ type: 'text', text: JSON.stringify({ success: true }, null, 2) }] };
                }
                case 'bulk_set': {
                    if (!updates?.length)
                        throw new Error('updates array required for bulk_set');
                    const result = await customFieldsClient.bulkSetFieldValues(updates, continue_on_error);
                    const set = result.results.filter((r) => r.status === 'set').length;
...
//# sourceMappingURL=custom-field-value-tools.js.map